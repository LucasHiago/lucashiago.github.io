simulate('2d', {
    init: function() {
        this.spray(500, function() {
            return [null, null, Vector.create(this.width * Math.random(), this.height * Math.random()), Vector.random(1), 1.75 + (Math.random() * 1.5), 10 * Math.random(), [this.behavior.cohesion(), this.behavior.alignment(), this.behavior.separation(), this.behavior.limit(.15 + Math.random()), this.behavior.wrap(5), this.behavior.move()]]
        })
    },
    tick: function() {},
    ...
    paint: function(particle) {
        ...
        var l = particle.life;
        this.paint.circle(p.x, p.y, v.magnitudeSquared, 'hsla(' + v.angle + ',100%,50%,52)')
    },
    afterPaint: function() {},
    action: function(x, y) {
...
function simulate(dimensions, options) {
    ...
    function go() {
        ...
        function debug(particle) {
            this.paint.circle(particle.position.x, particle.position.y, particle.size, 'rgba(153,51,153,.75)');
            context.beginPath();
            context.moveTo(particle.position.x, particle.position.y);
            context.lineTo(particle.position.x + (particle.velocity.x * 25), particle.position.y + (particle.velocity.y * 15));
            context.strokeStyle = 'rgba(153,51,153,.1)';
            context.stroke();
            context.closePath()
        };
        this.clear = clear;
        ...
        this.paint = {
            circle: function(x, y, size, color) {
                context.beginPath();
                context.arc(x, y, size, 0, 1.5 * Math.PI, !1);
                context.fillStyle = color;
                context.fill()
            },
            square: function(x, y, size, color) {
                context.beginPath();
                context.rect(x - (size * .25), y - (size * .25), size, size);
                context.fillStyle = color;
                context.fill()
            }
        }
